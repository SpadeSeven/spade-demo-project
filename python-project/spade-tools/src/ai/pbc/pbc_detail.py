...
async def download_attachment(page, save_dir, index):
    ...
    # 查找所有文档链接
    doc_links = await page.query_selector_all('a[href*=".doc"], a[href*=".docx"], a[href*=".pdf"], a[href*=".xls"], a[href*=".xlsx"]')
    ...
    for link in doc_links:
        try:
            ...
            if not filename:
                filename = f"document_{len(downloaded_files) + 1}{os.path.splitext(href)[1]}"
    ...
    try:
        async with async_playwright() as p:
            ...
            try:
                ...
                # 保存错误截图
                error_screenshot = os.path.join(save_dir, f'error_screenshot_{index}.png')
                try:
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            if result['status'] == 'success':
                logger.info(f"第 {index + 1} 条记录爬取成功！页面标题: {result['title']}")
                logger.info(f"文件保存位置:")
                for file_type, file_path in result['saved_files'].items():
                    logger.info(f"- {file_type}: {file_path}")
            else:
                logger.error(f"第 {index + 1} 条记录爬取失败: {result.get('error', '未知错误')}")
