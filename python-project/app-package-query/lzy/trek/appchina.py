...
def process(url, options, package_name):
    ...
    # cetagory
    category = ''
    other = etree.HTML(res.content).xpath('//*[@id="pagecontainer"]/div[3]/div[2]/div[2]/div[1]/div/p/text()')
    for i in other:
        ...
        if '分类' == i.split("：")[0]:
            ...
            break
    # appname
    appname = etree.HTML(res.content).xpath('//*[@id="pagecontainer"]/div[3]/div[1]/div[1]/div/h1[@class="app-name"]/text()')[
        0]
    # icon
    img_url = etree.HTML(res.content).xpath('//*[@id="pagecontainer"]/div[3]/div[1]/div[1]/div/img/@src')[0]
